<?php $i = 0; global $post; $id = $post->id;?>
	...
	<ul class="psp-documents-row">
		...
		while(has_sub_field('documents',$post_id)):
			...
			<li id="psp-project-<?php echo $post_id; ?>-doc-<?php echo $i; ?>" <?php if((psp_can_edit_project($id)) && (get_post_type() == 'psp_projects')) { echo 'class="psp-can-edit"'; } ?>>
                <form method="post" action="<?php echo get_permalink($post_id); ?>" class="document-update-form">
				    ...
				    <p class="psp-doc-title">
					    <a href="<?php echo $doc_link; ?>" target="_new"><strong><?php the_sub_field('title'); ?></strong></a>
