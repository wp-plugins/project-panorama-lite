 ...
 **/
    ...
    /* Setup Variables */
    $taskList = array();  $count = 0; $task_id = 0;
    ...
    while(has_sub_field('tasks',$id)):
        ...
        if((psp_can_edit_project($id)) && (get_post_type() == 'psp_projects')) {
        ...
        if ($taskCompleted == '100') { $task_class = 'complete'; } else { $task_class = ''; }

        $output .= '<li class="'.$task_class.' task-item task-item-'.$task_id.'" data-progress="'.get_sub_field('status',$id).'">';
        $output .= '<strong>'.get_sub_field('task').' '.$link.'</strong> <span><em class="status psp-'.get_sub_field('status',$id).'"></em></span>';
        if((psp_can_edit_project($id)) && (get_post_type() == 'psp_projects')) {
